...
var fall = setInterval(function() {
    ...
    if (jumping == 0) {
        if (birdTop < 450) {
            bird.style.top = (birdTop + 3) + "px";//adding 3 px continuously untill bird hits bottom of game container
        } else {
            endGame();
        }
    }
    ...
    var topPipeHeight = parseInt(window.getComputedStyle(topPipe).getPropertyValue("height"));
    var bottomPipeTop = topPipeHeight + 150; // Hole is 150px
...
function jump() {
    if (!gameRunning) return;//Exit if game isn't running
    
    jumping = 1;//Flag indicating bird currently jumping
    let jumpCount = 0;
    let jumpInterval = setInterval(function() {
        var birdTop = parseInt(window.getComputedStyle(bird).getPropertyValue("top"));
        if (birdTop > 20 && jumpCount < 15) {
            bird.style.top = (birdTop - 5) + "px";//Moving the bird up by 4px
        }
        jumpCount++;
        if (jumpCount > 20) {
            clearInterval(jumpInterval);
            jumping = 0;//to prevent overlapping jumps and again reset the jumping by the bird in the same game
        }
    }, 10);//Interval runs after 10ms
}
